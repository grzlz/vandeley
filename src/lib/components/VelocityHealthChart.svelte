<script>
	...
	function createVelocityChart() {
		...
		insights
			...
			.style('font-size', '12px')
			.text(
				`High Context Switching: ${data.contextSwitching.switchingRatio.toFixed(1)}% of sessions`
			);
...
<div class="w-full overflow-hidden">
	<div bind:this={chartContainer} class="flex h-72 w-full items-center justify-center">
		{#if !data}
			<div class="font-medium text-primary-500">No velocity health data available</div>
		{/if}
	</div>
</div>
