	...
	function createHeatmap() {
		...
		g.selectAll('.file-label')
			...
			.text((d) => {
				...
				return d.filename.length > maxLength ? '...' + d.filename.slice(-maxLength) : d.filename;
			});
		...
		if (width > 300) {
			const legendData = [
				...
				{
					label: 'Medium',
					color: colorScale(d3.max(topHotspots, (d) => d.totalModifications) * 0.6)
				},
...
<div class="w-full overflow-hidden">
	<div bind:this={chartContainer} class="flex w-full items-center justify-center overflow-x-auto">
		{#if !hotspots || hotspots.length === 0}
			<div class="p-8 font-medium text-primary-500">No file activity data available</div>
		{/if}
	</div>
</div>
