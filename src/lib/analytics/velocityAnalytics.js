...
function analyzeContextSwitching(sessions, fileHistory) {
    ...
    return {
        ...
        switchingRatio: sessions.length > 0 ? (highSwitchingSessions.length / sessions.length) * 100 : 0,
        topSwitchingSessions: contextSwitches
...
function analyzeOnboardingComplexity(contributors, commits) {
    ...
    contributors.forEach(contributor => {
        ...
        if (contributorCommits.length >= 5) { // Need minimum commits for analysis
    ...
    const averageRampUp = onboardingData.length > 0
...
function identifyKnowledgeExperts(contributors, fileHistory) {
    const experts = contributors.map(contributor => {
        const expertiseFiles = fileHistory.filter(file => 
            file.authors <= 2 && file.totalModifications > 5
        );
        
        const expertiseScore = expertiseFiles.length * 10 + 
            (contributor.totalCommits > 100 ? 20 : 0);
...
function calculateOverallBusFactor(fileBusFactor) {
    ...
    const averageBusFactor = busFactors.length > 0 
...
function findProductiveCommit(commits) {
    ...
    for (let i = 4; i < commits.length; i++) { // Start from 5th commit
...
function identifyComplexityFactors(onboardingData, commits) {
    ...
    const avgRampUp = onboardingData.reduce((sum, d) => sum + d.rampUpDays, 0) / onboardingData.length;
...
function generateVelocityRecommendations(metrics) {
    ...
    if (metrics.contextSwitching.penaltyScore > 60) {
        recommendations.push({
            ...
            action: 'Organize work by feature verticals rather than technical layers to reduce context switching'
        });
    }
    ...
    if (metrics.onboardingComplexity.complexityScore > 60) {
        recommendations.push({
            ...
            action: 'Create documentation, setup guides, and identify good "first issues" for new contributors'
        });
    }
    ...
    if (metrics.velocityScore < 50) {
        recommendations.push({
            ...
            action: 'Focus on reducing context switching, knowledge sharing, and improving developer onboarding'
        });
    }
