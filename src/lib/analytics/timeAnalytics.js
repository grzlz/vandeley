export function analyzeCommitTiming(commits) {
    ...
    const afterHours = hourlyActivity.slice(18).concat(hourlyActivity.slice(0, 9)).reduce((sum, count) => sum + count, 0);
...
export function analyzeCommitVelocity(commits) {
    ...
    const averageTimeBetweenCommits = velocityData.reduce((sum, data) => 
        sum + data.timeBetweenCommits, 0) / velocityData.length;
    
    const averageChangesPerCommit = commits.reduce((sum, commit) => 
        sum + commit.insertions + commit.deletions, 0) / commits.length;
    
    return {
        velocityData,
        averageTimeBetweenCommits,
        averageChangesPerCommit,
        totalCommitDays: new Set(commits.map(c => 
