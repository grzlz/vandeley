export function analyzeTechnicalDebt(commits, projectEvolution) {
	...
	const top10PercentChanges = hotspots
	...
	const hotspotConcentrationRatio =
		totalChanges > 0 ? (top10PercentChanges / totalChanges) * 100 : 0;

	// Code Churn Rate Analysis
	const churnData = fileHistory
		.map((file) => {
	...
	const averageChurnRate =
		churnData.reduce((sum, file) => sum + file.churnRate, 0) / churnData.length || 0;
	...
	const refactoringRatio =
		commitPatterns.totalCommits > 0
...
function calculateArchitectureViolations(fileHistory) {
	...
	return {
		...
		moduleDistribution: moduleNames
			.map((name) => ({
				module: name,
