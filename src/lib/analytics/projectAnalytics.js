	...
	sortedCommits.forEach((commit, index) => {
		...
		if (index % Math.ceil(sortedCommits.length / 20) === 0) {
			evolutionTimeline.push({
				...
				netLines: cumulativeInsertions - cumulativeDeletions,
				dominantAuthor: getMostActiveAuthorInPeriod(
					sortedCommits.slice(Math.max(0, index - 50), index + 1)
				)
			});
		}
	});
	...
	return {
		...
		projectAge:
			sortedCommits.length > 0
				? sortedCommits[sortedCommits.length - 1].timestamp - sortedCommits[0].timestamp
				: 0
	};
}
...
function getMostActiveAuthorInPeriod(commits) {
	...
	return Object.keys(authorCounts).reduce(
		(a, b) => (authorCounts[a] > authorCounts[b] ? a : b),
		'Unknown'
	);
}
