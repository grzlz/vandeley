...
function analyzeCouplingEvolution(commits, projectEvolution) {
	...
	const earlierCoupling =
		couplingTrend.slice(-6, -3).reduce((sum, t) => sum + t.couplingRatio, 0) / 3;
...
function analyzePerformanceDebt(commits, fileHistory) {
	...
	const performanceFiles = fileHistory.filter((file) => isPerformanceRelatedFile(file.filename));

	const performanceCommits = commits.filter(
		(commit) =>
	...
	const debtScore = calculatePerformanceDebtScore(
		performanceFiles,
		performanceCommits,
		commits.length
	);
...
function analyzeFeatureFlagDebt(commits) {
	...
	const flagDebtRatio =
		flagIntroductions.length > 0 ? 1 - flagRemovals.length / flagIntroductions.length : 0;
...
function assessMVPMaturity(commits, contributors, projectEvolution) {
	...
	const maintenanceCommits = commits.filter(
		(commit) =>
	...
	const featureMaintenanceRatio =
		featureCommits.length > 0 ? maintenanceCommits.length / featureCommits.length : 0;

	// MVP maturity indicators
	const maturityIndicators = {
		codebaseStability:
			featureMaintenanceRatio < 0.5 ? 80 : Math.max(0, 80 - featureMaintenanceRatio * 100),
		teamGrowth: teamSize > 1 ? Math.min(100, teamSize * 20) : 20,
		developmentPace: Math.min(
			100,
			Math.max(0, 100 - (developmentDuration > 90 ? developmentDuration - 90 : 0))
		),
		codebaseSize: codebaseSize > 50 ? Math.min(100, codebaseSize / 2) : codebaseSize * 2
	};

	const maturityScore =
		Object.values(maturityIndicators).reduce((sum, score) => sum + score, 0) /
		Object.keys(maturityIndicators).length;
...
function createCouplingTimeSlices(commits, daysPerSlice) {
	...
	for (let time = startTime; time <= endTime; time += sliceDuration) {
		const sliceCommits = sortedCommits.filter(
...
function isPerformanceRelatedFile(filename) {
	...
	return performanceIndicators.some((indicator) => filename.toLowerCase().includes(indicator));
}
...
function isFeatureFlagRelated(message) {
	...
	return flagKeywords.some((keyword) => message.toLowerCase().includes(keyword));
}

function isFeatureFlagFile(filename) {
	const flagIndicators = ['flag', 'toggle', 'experiment', 'config'];
	return flagIndicators.some((indicator) => filename.toLowerCase().includes(indicator));
}
...
function isInfrastructureFile(filename) {
	...
	return (
		infraIndicators.some((indicator) => filename.toLowerCase().includes(indicator)) ||
		filename.startsWith('.')
	);
}
...
function assessScalingReadiness(indicators) {
	const avgScore =
		Object.values(indicators).reduce((sum, score) => sum + score, 0) /
