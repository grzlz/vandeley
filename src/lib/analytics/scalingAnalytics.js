export function analyzeScalingReadiness(commits, sessions, contributors, projectEvolution) {
    ...
    const coordinationOverhead = calculateCoordinationOverhead(projectEvolution.fileHistory, contributors);
...
function calculateCoordinationOverhead(fileHistory, contributors) {
    ...
    const overheadScore = Math.min(100, 
        (sharedFileRatio * 0.6) + (highContentionRatio * 1.5) + 
        Math.min(50, totalCoordinationCost / 1000)
    );
...
function createTimeSlices(commits, daysPerSlice) {
    ...
    for (let time = startTime; time <= endTime; time += sliceDuration) {
        const sliceCommits = commits.filter(commit => 
...
function calculateVelocityTrend(timeSlices) {
    ...
    const recentAvg = recentSlices.reduce((sum, slice) => sum + slice.commitCount, 0) / recentSlices.length;
    const earlierAvg = earlierSlices.reduce((sum, slice) => sum + slice.commitCount, 0) / earlierSlices.length;
...
function analyzeCouplingTrends(fileHistory, commits) {
    ...
    const couplingScore = fileHistory.reduce((score, file) => {
...
function generateScalingRecommendations(metrics) {
    ...
    if (metrics.scalingReadinessScore < 40) {
        recommendations.push({
            ...
            description: 'Multiple factors indicate the codebase may struggle with team and product scaling.',
