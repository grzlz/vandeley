...
<script>
	...
	// Check for git log content from sessionStorage (from landing page upload)
	$effect(() => {
		if (typeof window !== 'undefined') {
			const storedContent = sessionStorage.getItem('gitLogContent');
			if (storedContent) {
				sessionStorage.removeItem('gitLogContent');
				analyzeGitLog(storedContent);
			}
		}
	});
